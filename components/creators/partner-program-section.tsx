...
export function PartnerProgramSection() {
  ...
  const [submittedData, setSubmittedData] = useState({ orderId: "", handle: "", name: "" })
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      const saved = localStorage.getItem("upforge_creator_verification_data")
      if (saved) {
        try {
  ...
  const handleVerifyOrder = (e: React.FormEvent) => {
    ...
    if (!trimmedOrderId || !trimmedHandle || !trimmedName) {
      setErrorMsg("Please fill in your Order ID, social media handle, and name.")
      return
    }

    const data = { orderId: trimmedOrderId, handle: trimmedHandle, name: trimmedName }
    setIsSubmitted(true)
    setSubmittedData(data)

    if (typeof window !== "undefined") {
      localStorage.setItem("upforge_creator_verification_data", JSON.stringify(data))
    }

    // Direct user to WhatsApp with prefilled message
    const message = encodeURIComponent(
      `Hi UpForge! I'd like to verify my purchase for the Partner Program.\n\nName: ${trimmedName}\nHandle: ${trimmedHandle}\nAmazon Order ID: ${trimmedOrderId}`
    )
    window.open(`https://api.whatsapp.com/send?phone=&text=${message}`, "_blank")
  }
...
2. VERIFY YOUR PURCHASE
   - Message our Program Verification desk on WhatsApp (wa.link/gmntyi) with:
     * Amazon Order ID
     * Social Media Handle (@username)
     * Your Full Name

3. GET APPROVED (SLA: WITHIN 1-2 DAYS)
   - Our Program Manager checks the Order ID against genuine purchase records.
...
5. PAYOUTS & DISQUALIFICATION RULES
   ...
   - Strict Anti-Fraud Policy: Views from bought sources, click farms, bot networks, or reciprocal engagement will lead to immediate disqualification and permanent removal.
...
`
  ...
  return (
    <section className="border-b border-border bg-gradient-to-b from-amber-50/70 via-background to-background dark:from-amber-950/20 dark:via-background dark:to-background">
      <div className="max-w-[1300px] mx-auto px-4 md:px-8 py-10 md:py-16 space-y-16">
        ...
        {/* 3. HOW IT WORKS + VERIFICATION WIDGET */}
        <div className="grid lg:grid-cols-12 gap-8 items-start">
              ...
              <div className="p-5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm flex items-start gap-4">
                ...
                <div>
                  ...
                  <a
                    ...
                    className="inline-flex items-center gap-1 text-xs text-amber-600 dark:text-amber-400 font-bold hover:underline mt-2"
                  >
              ...
              <div className="p-5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm flex items-start gap-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-slate-600 dark:text-slate-300 leading-relaxed mb-2">
                    Message our Program Verification desk on WhatsApp with three things: your Amazon <strong className="text-slate-900 dark:text-white">Order ID</strong>, your <strong className="text-slate-900 dark:text-white">social media handle</strong>, and your <strong className="text-slate-900 dark:text-white">name</strong>.
                  </p>
                  <a
                    href={WHATSAPP_LINK}
                    target="_blank"
                  ...
                  >
                    ...
                    <span>WhatsApp Verification: wa.link/gmntyi</span>
                  </a>
                </div>
              </div>
          ...
          {/* RIGHT: INTERACTIVE VERIFICATION WIDGET (5 cols) */}
          <div className="lg:col-span-5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 md:p-8 flex flex-col justify-between shadow-xl sticky top-24">
            <div>
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="inline-flex items-center gap-1 text-[10px] font-semibold text-slate-500 font-mono">
                  <Clock className="w-3 h-3 text-amber-500" /> SLA: Within 1–2 days
                </span>
              </div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 mb-6 leading-relaxed">
                Provide your Order ID, handle, and name below to generate your WhatsApp verification message automatically.
              </p>
                ...
                <form onSubmit={handleVerifyOrder} className="space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-3.5 py-2.5 text-xs bg-slate-50 dark:bg-slate-800/80 border border-slate-300 dark:border-slate-700 rounded-xl text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-amber-500"
                    />
                  </div>
                  ...
                  >
                    <MessageCircle className="w-4 h-4" />
                    <span>Verify via WhatsApp (wa.link/gmntyi)</span>
                  </button>
                </form>
              ) : (
                <div className="space-y-4">
                  <div className="p-4 rounded-2xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-800 dark:text-emerald-300 space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-mono font-bold uppercase tracking-wider flex items-center gap-1.5">
                        <CheckCircle2 className="w-4 h-4 text-emerald-500" /> Submitted
                      </span>
                      <span className="text-[10px] font-bold bg-emerald-500/20 px-2 py-0.5 rounded text-emerald-700 dark:text-emerald-300">
                        1–2 Days SLA
                      </span>
                    </div>
                    <p className="text-xs leading-relaxed">
                      Verification request submitted for <strong className="font-mono">{submittedData.handle}</strong> (Order ID: <span className="font-mono font-bold">{submittedData.orderId}</span>).
                    </p>
                  </div>
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="w-full py-3 bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs uppercase tracking-wider rounded-xl transition flex items-center justify-center gap-2"
                  >
                    <MessageCircle className="w-4 h-4" />
                  ...
                  <button
                    onClick={() => {
                      ...
                      localStorage.removeItem("upforge_creator_verification_data")
                    }}
                    className="text-[11px] text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 underline font-mono block mx-auto"
                  >
                    Submit different details
                  </button>
                </div>
              )}
            </div>
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl max-w-2xl w-full p-6 md:p-8 shadow-2xl relative max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4 text-xs text-slate-600 dark:text-slate-300 leading-relaxed font-sans">
              ...
              <ul className="space-y-3 list-disc pl-4">
                ...
                <li><strong>WhatsApp Verification:</strong> Send your Order ID, handle, and name to wa.link/gmntyi. Processing SLA is 1–2 days.</li>
